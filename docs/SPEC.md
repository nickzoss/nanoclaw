...
| Agent | @github/copilot (GitHub Copilot CLI) | Run the agent with tools and MCP servers |
...
The token is used by the GitHub Copilot CLI (`copilot`) inside the container. You can also use OneCLI to manage credentials via the OneCLI gateway — run `onecli --help` for details.

Only `GITHUB_TOKEN` (and `GH_TOKEN` as an alias) are extracted from `.env` and written to `data/env/env`, then mounted into the container at `/workspace/env-dir/env` and sourced by the entrypoint script. This ensures other environment variables in `.env` are not exposed to the agent. This workaround is needed because some container runtimes lose `-e` environment variables when using `-i` (interactive mode with piped stdin).
